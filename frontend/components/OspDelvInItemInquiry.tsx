...
export type OspDelvInItemRow = {
  ...
  /** 작업지시번호(백엔드 `OspInstructionNo` — 라인·헤더 작업지시 후보 COALESCE) */
...
/** 순번 + 데이터 열 30개 (납품반품구분·작업지시번호·ERP품명/품번/규격 등) */
const COL_COUNT = 31;

export type OspDelvInItemInquiryProps = {
  embedded?: boolean;
};
...
export function OspDelvInItemInquiry({ embedded = false }: OspDelvInItemInquiryProps) {
  ...
  const [items, setItems] = useState<OspDelvInItemRow[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/erp/osp-delv-in-items?${params}`, { credentials: 'include' });
      if (!res.ok) {
        setItems([]);
        setTruncated(false);
      ...
      const body = (await res.json()) as { items: OspDelvInItemRow[]; truncated: boolean };
            ...
            <table className="min-w-max border-collapse">
              <thead className="sticky top-0 z-10 bg-[#E7E6E6]">
                <tr>
                  ...
                  <th className={EIS_TH}>외주입고번호</th>
                  <th className={EIS_TH}>외주입고일</th>
                  <th className={EIS_TH}>외주거래처</th>
                  <th className={EIS_TH}>입고부서</th>
                  <th className={EIS_TH}>입고담당자</th>
                  <th className={EIS_TH}>납품반품구분</th>
                  <th className={EIS_TH}>작업지시번호</th>
                  <th className={EIS_TH}>ERP품명</th>
                  <th className={EIS_TH}>ERP품번</th>
                  <th className={EIS_TH}>ERP규격</th>
                  <th className={EIS_TH}>입고수량</th>
                  <th className={EIS_TH}>단가</th>
                  <th className={EIS_TH}>금액</th>
                  <th className={EIS_TH}>부가세</th>
                  <th className={EIS_TH}>부가세포함여부</th>
                  <th className={EIS_TH}>합계</th>
                  <th className={EIS_TH}>입고창고</th>
                  <th className={EIS_TH}>특이사항</th>
                  <th className={EIS_TH}>비고</th>
                  <th className={EIS_TH}>Lot No.</th>
                  <th className={EIS_TH}>차수</th>
                  <th className={EIS_TH}>PO번호</th>
                  <th className={EIS_TH}>자재투입여부</th>
                  <th className={EIS_TH}>원자재단가</th>
                  <th className={EIS_TH}>품목대분류</th>
                  <th className={EIS_TH}>품목중분류</th>
                  <th className={EIS_TH}>품목소분류</th>
                  <th className={EIS_TH}>최종작성일시</th>
                  <th className={EIS_TH}>최종작성자</th>
                  <th className={EIS_TH}>전표번호</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr>
                    <td className={EIS_TD} colSpan={COL_COUNT}>
                      {error
                        ? '—'
                        : hasFetched
                          ? '해당 기간에 조회된 데이터가 없습니다. 일자·법인(ERP_OSP_DELV_IN_COMPANY_SEQ)를 확인하세요.'
                ...
                {items.map((r, idx) => (
                  <tr key={`${r.receiptNo ?? ''}-${r.lineSerl ?? idx}-${idx}`}>
                    ...
                    <td className={EIS_TD_LEFT}>{cellText(r.deptName)}</td>
                    <td className={EIS_TD}>{cellText(r.inChargeEmpName)}</td>
                    <td className={EIS_TD}>{cellText(r.receiptKind)}</td>
                    <td className={EIS_TD}>{cellText(r.ospInstructionNo)}</td>
                    ...
                    <td className={EIS_TD}>{cellText(r.vatIncludedFlag)}</td>
                    <td className={EIS_TD_NUM}>{formatNumber(r.totalAmount)}</td>
                    <td className={EIS_TD_LEFT}>{cellText(r.whName)}</td>
                    <td className={EIS_TD_LEFT}>{cellText(r.specialNote)}</td>
                    <td className={EIS_TD_LEFT}>{cellText(r.remark)}</td>
                    <td className={EIS_TD}>{cellText(r.lotNo)}</td>
                    <td className={EIS_TD_NUM}>{formatNumber(r.lineDegree)}</td>
                    <td className={EIS_TD}>{cellText(r.purOrderNo)}</td>
                    <td className={EIS_TD}>{cellText(r.matInputYn)}</td>
                    <td className={EIS_TD_NUM}>{formatNumber(r.rawMatUnitPrice)}</td>
                    <td className={EIS_TD}>{cellText(r.itemCatLarge)}</td>
                    <td className={EIS_TD}>{cellText(r.itemCatMid)}</td>
                    <td className={EIS_TD}>{cellText(r.itemCatSmall)}</td>
                    <td className={EIS_TD}>{cellText(r.lastModifiedAt)}</td>
                    <td className={EIS_TD}>{cellText(r.lastWriterName)}</td>
                    <td className={EIS_TD}>{cellText(r.slipNo)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
